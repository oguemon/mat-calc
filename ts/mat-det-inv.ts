...
$("#calc").on("click", function () {
  ...
  if (input_error) {
    $("#error_msg_box").html(
      '<div id="error_msg">正しく入力して下さい！</div>'
    );
    return;
  }
  ...
  for (let j = 0; j < row; j++) {
    ...
    for (let i = rank + 1; i < line; i++) {
      ...
      if (A.val[i][j].isZero() == false) {
        step_count++;
        const mlt: Mynum = Mynum.mul(
          new Mynum(-1),
          Mynum.div(A.val[i][j], A.val[rank][j])
        );
  ...
  function showMatrixTriA(triA_showing_index: number) {
    const ele_matrix_triA_result: JQuery<HTMLElement> = $(
      "#matrix_triA_result"
    );
    const ele_matrix_triA_operation_content: JQuery<HTMLElement> = $(
      "#matrix_triA_operation_content"
    );
    const ele_matrix_triA_operation_step: JQuery<HTMLElement> = $(
      "#matrix_triA_operation_step"
    );

    ele_matrix_triA_result.html(
      Mat.toMathJax(stepsA[triA_showing_index].result, "A")
    );
    ...
    if (triA_showing_index > 0) {
      ...
      const multipulator_latex: string = Mynum.toLatex(
        stepsA[triA_showing_index].multipulator
      );
      if (stepsA[triA_showing_index].is_swap == true) {
        ele_matrix_triA_operation_content.html(
          line1 + "行目と" + line2 + "行目を入れ替え"
        );
      } else {
        ele_matrix_triA_operation_content.html(
          line1 +
        ...
        MathJax.Hub.Typeset(
          ele_matrix_triA_operation_content[0],
          function () {}
        );
      }
    } else {
  ...
  if (rank == A.lines) {
    $("#matrix_rankA_reversible").html(
      "階数と行列の列数が等しいので逆行列があります。"
    );
  } else {
    $("#matrix_rankA_reversible").html(
      "階数と行列の列数が等しくないので逆行列がありません……"
    );
  }
  ...
  if (N == rank) {
    ...
    // 逆行列がない旨の表示
    ele_matrix_revA.html(
      "逆行列はありません（階数と正方行列の次数が異なるから）"
    );
  }
  ...
  // 移動先を数値で取得(ゆとり分だけ引く)
  const headline_result_offset: JQuery.Coordinates | undefined =
    $("#panel-header").offset();
  const position: number =
    headline_result_offset != undefined ? headline_result_offset.top - 10 : 0;
  // スムーススクロール
